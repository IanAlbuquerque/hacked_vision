...
float            YawV(3.141592f);       // Vertical rotation of the player
Vector3f         Pos(0.0f,0.0f,0.0f); // Position of player
Leap::Controller controller;

Vector3f screen_position = Vector3f(0.0f , 1.5f, 2.0f);
float old_x = 0;
bool use_old_x = false;
float old_y = 0;
bool use_old_y = false;
float old_z = 0;
bool use_old_z = false;
...
int WINAPI WinMain(HINSTANCE hinst, HINSTANCE, LPSTR, int)
{
    ...
    while (!(WND.Key['Q'] && WND.Key[VK_CONTROL]) && !WND.Key[VK_ESCAPE])
    {
        ...
        for (int eye = 0; eye < 2; eye++)
        {
			...
			#else
				WND.ClearAndSetRenderTarget(useBuffer->TexRtv, pEyeDepthBuffer[eye], Recti(EyeRenderViewport[eye]));
				WebCamMngr.DrawLookThrough(eye);
				WND.SetRenderTarget(useBuffer->TexRtv, pEyeDepthBuffer[eye], Recti(EyeRenderViewport[eye]));
			#endif
            ...
            if (updateEyeImage)
			{
                ...
                *useYaw     = Yaw;
				...
				Matrix4f _rollPitchYaw = Matrix4f::RotationY(Yaw);
				Matrix4f _finalRollPitchYaw = _rollPitchYaw * Matrix4f(useEyePose->Orientation);
				Vector3f _finalUp = _finalRollPitchYaw.Transform(Vector3f(0, 1, 0));
				Vector3f _finalForward = _finalRollPitchYaw.Transform(Vector3f(0, 0, -1));
				Vector3f _shiftedEyePos = _rollPitchYaw.Transform(Vector3f(0, 0, 0));

				Matrix4f _view = Matrix4f::Identity();
				_view.SetZBasis(Vector3f(0, 0, -1));
				_view.SetXBasis(Vector3f(-1, 0, 0));
				Matrix4f _proj = ovrMatrix4f_Projection(EyeRenderDesc[eye].Fov, 0.2f, 1000.0f, true);

				Matrix4f rollPitchYaw = Matrix4f::RotationX(YawV) * Matrix4f::RotationY(Yaw);
				...
				*/
				...
				if (controller.isConnected())
				{
					...
					if (!hand_list.isEmpty())
					{
						Leap::Hand hand = hand_list[0];
						Leap::Vector palm_position = hand.palmPosition();
						float r = hand.sphereRadius();
						Vector3f hand_position = Vector3f(palm_position.x / 10.0f, -palm_position.z / 10.0f, palm_position.y / 10.0f);
						...
						else
						{
							...
							/*
							Matrix4f inv_proj = _proj.InvertedHomogeneousTransform();
							Matrix4f inv_view = _view.InvertedHomogeneousTransform();
							Matrix4f inv;
							Matrix4f::Multiply(&inv, inv_proj, inv_view);
							Matrix4f go;
							Matrix4f::Multiply(&go, view, proj);
							Matrix4f res;
							Matrix4f::Multiply(&res, go, inv);


							screen_position = res.Transform(hand_position);
							*/
							if (!use_old_x)
							{
								old_x = hand_position.x;
								old_y = hand_position.y;
								old_z = hand_position.z;
							}
							else
							{
								Yaw -= (hand_position.x - old_x)*0.1f;
								YawV -= (hand_position.y - old_y)*0.1f;
								screen_position.z += (hand_position.z - old_z)*0.1f;
								old_x = hand_position.x;
 								old_y = hand_position.y;
								old_z = hand_position.z;
							}
						...
						roomScene.Models[0]->Pos = hand_position;
						//roomScene.Models[1]->Pos = right_position;

						//for (int t = 0; t<timesToRenderScene; t++)
							//roomScene.Render(_view, _proj.Transposed());
					}
					else
					{
						use_old_x = false;
				...
				WebCamMngr.WebCams[1].DrawBoard(view, proj.Transposed(), screen_position.x, screen_position.y, screen_position.z);
				/*
				}
				else 
				{

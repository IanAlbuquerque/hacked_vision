...
struct DirectX11
{
	...
	void SetRenderTarget(ID3D11RenderTargetView * rendertarget, ImageBuffer * depthbuffer, Recti vp);
...
struct Scene  
{
    ...
    Scene(int reducedVersion) : num_models(0) // Main world
    {
        ...
        m->AddSolidColorBox( -1.0, -1.0, 0,  +1.0f,  +1.0f, +1.0f,  Model::Color(64,64,64)); 
...
//----------------------------------------------------------------------------------------------------------
void DirectX11::ClearAndSetRenderTarget(ID3D11RenderTargetView * rendertarget,ImageBuffer * depthbuffer, Recti vp)
{
    float black[] = {0, 0, 0, 1}; 
...
void DirectX11::SetRenderTarget(ID3D11RenderTargetView * rendertarget, ImageBuffer * depthbuffer, Recti vp)
{
	float black[] = { 1, 0, 0, 1 };
	Context->OMSetRenderTargets(1, &rendertarget, depthbuffer->TexDsv);
	//Context->ClearRenderTargetView(rendertarget, black);

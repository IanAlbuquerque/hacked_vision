...
#define WEBCAM_0_DEVICE_NUMBER			4		// The device number for webcam 0 (eg.: Left Eye) among connected ones. If you have 2 webcams and they are inverted, swith the number with WEBCAM_1_DEVICE_NUMBER!
#define WEBCAM_0_VERT_ORIENTATION		false	// Is webcam 0 (eg.: Left Eye) vertically positioned?
#define WEBCAM_0_HMD_FOV_RATIO			0.7f	// The ratio: (Web Cam Diagonal Field of View) / (Oculus Rift Eye Field of View) for webcam 0 (eg.: Left Eye)
#define WEBCAM_1_DEVICE_NUMBER			2		// The device number for webcam 1 (eg.: Right Eye) among connected ones. If you have 2 webcams and they are inverted, swith the number with WEBCAM_0_DEVICE_NUMBER!
#define WEBCAM_1_VERT_ORIENTATION		false	// Is webcam 1 (eg.: Right Eye) vertically positioned?
...
public:
	...
	void DrawBoard(Matrix4f view, Matrix4f proj, float x = 0, float y = 0, float z = 0, float fXBoardGap = 0)
	{
		if(!pQuadModel) { return; }

		
		Matrix4f modelmat = Matrix4f::Translation(x, y, z) *
							Matrix4f::Scaling(1.0f, (bIsVertOriented ? 1.0f/fAspectRatio : fAspectRatio), 1.0f) * 
							Matrix4f::RotationZ(bIsVertOriented ? MATH_FLOAT_PIOVER2 + MATH_FLOAT_PI : 2 * MATH_FLOAT_PI);
